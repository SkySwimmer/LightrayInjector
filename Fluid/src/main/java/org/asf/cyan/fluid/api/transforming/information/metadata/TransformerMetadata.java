...
public abstract class TransformerMetadata {
	...
	protected synchronized void dumpErrorBacktraceInternal(String message, StackTraceElement[] elements, File output,
		...
		dumpBacktraceNoStacktrace(output, pool, classesNew, str -> log.debug(str), str -> log.warn(str),
				str -> log.warn(str), (str, e) -> log.error(str, e));
	}
		...
		if (classesNew) {
			...
			for (TransformerMetadata md : getLoadedTransformers()) {
				try {
					...
					log.error("Transformer class dump failed, transformer: " + md.getTransfomerClass(), e);
				}
			}
	...
	protected static void setImplementation(TransformerMetadata implementation) {
		log.debug("Assigning FLUID Transformer Metadata Implementation... Using the "
