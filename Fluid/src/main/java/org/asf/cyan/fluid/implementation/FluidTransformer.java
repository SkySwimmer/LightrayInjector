...
/**
 * 
 * Fluid implementation of the FLUID modification engine.
 * 
 * @author Stefan0436 - AerialWorks Software Foundation
 *
 */
public class FluidTransformer extends Transformer {
	...
	protected void applyInjectAt(TransformContext context, TargetInfo targetInfo, MethodNode target,
					...
					if (target.localVariables.size() == 0) {
						...
						for (int i = 0; i < mthLocalVars.length; i++) {
							...
							while (start != null) {
								start = start.getPrevious();
								if (start instanceof LabelNode) {
									vars[i + index].start = (LabelNode) start;
									break;
								}
							}
						}
		...
		if (injectNode != null) {
			...
			if (targetInfo.location != InjectLocation.HEAD || targetInfo.offset > 0)
				log.warn("Could not apply transformer " + context.transformerType + " to method " + transformer.name
