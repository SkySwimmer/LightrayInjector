...
public abstract class Transformer {
	...
	protected static Logger log = LogManager.getLogger("Fluid");
	...
	protected synchronized void runTransformers(ClassNode target, String clName, String loadingName,
			Map<String, String> transformerOwners, Map<String, ArrayList<ClassNode>> transformers,
		...
		for (ClassNode transformer : arr) {
			...
			try {
				log.debug("Applying transformer " + typeName + " to class "
				...
				if (Modifier.isInterface(transformer.access)) {
					...
					for (MethodNode meth : transformer.methods) {
						...
						if (AnnotationInfo.isAnnotationPresent(Erase.class, meth)) {
							...
							log.debug("Transforming method: " + targetNode.name + "... (using transformer method: "
									+ meth.name + ")");
							...
							log.debug("Created method " + ninfo.name);
							transformedMethods.add(ninfo.name + " " + ninfo.toDescriptor() + " " + clName + " "
						...
						try {
							...
							for (Method meth : transformerCls.getMethods()) {
								if (Modifier.isStatic(meth.getModifiers()) && meth.isAnnotationPresent(ASM.class)) {
									log.debug("Transforming " + clName + " with ASM-based transformer " + meth.getName()
									...
									for (Parameter param : meth.getParameters()) {
										if (param.getType().getTypeName().equals(FluidClassPool.class.getTypeName())
											...
											log.error("Unable to run @ASM transformer method: " + meth.getName()
											...
											log.error(
													" - FluidClassPool     - class pool used to load the transformers and classes");
									...
									if (!error) {
										try {
											...
											log.error("Failed to transform with ASM transformer " + meth.getName()
							...
							log.error(
									"Failed to load transformer " + typeName + " as a class, needed for @ASM methods.",
									e);
						}
					}
				}
				...
				log.fatal("FLUID transformation failed! Transformer: " + typeName, ex);
				...
				try {
					...
					log.error("Could not dump FLUID transformer metadata, an exception was thrown.", e);
				}
				System.exit(1);
			}
		}
	...
	protected static void setImplementation(Transformer transformerEngine) {
		log.debug("Assigning FLUID Modification Engine... Using the " + transformerEngine.getImplementationName()
				+ " Modification Engine...");
	 ...
	 */
	public static byte[] transform(ClassNode cls, Map<String, String> transformerOwners,
			Map<String, ArrayList<ClassNode>> transformers, String clName, String loadingName, FluidClassPool pool,
